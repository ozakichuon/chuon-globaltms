...
function fetchGviz(sheetName: string): Promise<any> {
...
function parseAddress(addr: string) {
  ...
  if (!dorm) return null;
  let room = "";
  if (dorm.keyword === "アトムスカイマンション") { const m = a.match(/アトムスカイマンション(\S+)/); room = m ? m[1] : ""; }
  else if (dorm.keyword === "ビレッジハウス松山上野") { const m = a.match(/ビレッジハウス松山上野(\S+)/); room = m ? m[1] : ""; }
  else if (dorm.keyword === "コーポ津吉") { const m = a.match(/コーポ津吉(\S+)/); room = m ? m[1] : ""; }
  else if (dorm.keyword === "東温市牛渕") { room = a.replace(/^.*東温市牛渕/, "東温市牛渕"); }
  else if (dorm.keyword === "EARLS") { const m = a.match(/(\d+)号/); room = m ? m[1] : a.replace(/.*COURT東温[ⅡII]+\s*/, ""); }
  else if (dorm.keyword === "シティハイム") { const m = a.match(/シティハイム(\S+)/); room = m ? m[1] : ""; }
  else if (dorm.keyword === "中野町") { room = a; }
  else if (dorm.keyword === "HAPPINESS") { const m = a.match(/HAPPINESS\s*(F棟\S+)/i); room = m ? m[1] : a.replace(/.*HAPPINESS\s*/i, ""); }
  return { dormitory_id: dorm.id, dormitory_name: dorm.name, room_no: room || null };
}

function stableId(base: string) {
  let h = 0x811c9dc5;
...
export async function POST(req: Request) {
  ...
  try {
    ...
    let existingPhotoMap: Record<string, string> = {};
    try {
      const existing = JSON.parse(fs.readFileSync(path.join(process.cwd(), "src/lib/data/employees.json"), "utf-8"));
    ...
    for (const row of empTable.rows) {
      ...
      const filename = photoPath ? photoPath.split("/").pop() : "";
      const photoUrl = (filename && photoMapByFilename[filename]) ? photoMapByFilename[filename] : (existingPhotoMap[code] || null);
      employees.push({
        id: code, employee_code: code, display_name: s(c[7]), furigana: s(c[6]) || null,
        nickname: s(c[8]) || null, nationality: s(c[3]), nationality_jp: s(c[3]) || null,
        gender: toGender(s(c[9])), birth_date: s(c[10]) || null, marital_status: null,
        support_agency: null, hired_at: s(c[14]) || null, expected_return: null,
        visa_type_jp: visaTypeJp || null, visa_status: toVisaStatus(visaTypeJp),
    ...
    const genderByCode: Record<string, string> = {};
    ...
    for (const row of empTable.rows) {
      ...
      assignments.push({
        id: assignId, dormitory_id: parsed.dormitory_id, dormitory_name: parsed.dormitory_name,
        room_id: roomId, room_no: parsed.room_no, resident_name: s(c[6]) || s(c[7]),
        employee_id: assignId, employee_code: code, photo_url: photoUrl, rent_burden: null,
        workplace: s(c[2]) || null, nationality: s(c[3]) || null, visa_type: s(c[16]) || null,
    ...
    // tickets
    const tickets: any[] = [];
    for (const row of tickTable.rows) {
      const c = row.c;
      if (!c || !c[0] || !c[0].v) continue;
      const empCode = s(c[4]);
      const responses: any[] = [];
